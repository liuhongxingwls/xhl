...
/* DIALOG */
var dialog = {
    alertTpl : function(title,msg){
        return this.confirmTpl(title,msg);
    },
    confirmTpl : function(title,msg){
        return '<div class="modal" tabindex="-1" role="dialog" aria-labelledby="defModalHead" aria-hidden="true">'
            +'<div class="modal-dialog">'
            +'<div class="modal-content">'
            +    '<div class="modal-header">'
            +        '<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span><span class="sr-only">Close</span></button>'
            +        '<h4 class="modal-title" id="defModalHead">'+title+'</h4>'
            +    '</div>'
    ...
    alert : function(msg,type,callback,options){
        var defaultTitle = type ? '提示' : '错误';
        var title = ('undefined' !== typeof options && 'undefined' !== typeof options.title) ? options.title : defaultTitle;
        var box = this.alertTpl(title,msg);
        var option = {
            buttons : [
                {
                    text : '确定',
        ...
        var mergeOption = $.extend(true,{},options,option);
        this.createDialog(box,mergeOption);
    },
    confirm : function(msg,callback,options){
        var title = ('undefined' !== typeof options && 'undefined' !== typeof options.title) ? options.title : '确认操作';
        var box = this.confirmTpl(title,msg);
        var option = {
            buttons : [
                {
                    text : '确定',
    ...
    createDialog : function(box,options){
        var buttons = this.getButtons(options);
        var obj = $(box);
        for (var i in buttons){
            obj.find('.modal-footer').append(buttons[i]);
        }
        obj.on('hide.bs.modal',function(){
            $(this).remove();
        });
        obj.appendTo(document.body).modal(options);
    },
    getButtons : function(conf){
        var arr = {};
        for(var i in conf.buttons){
            var btn = "<button class='"+conf.buttons[i].className+"'>";
            btn += conf.buttons[i].text;
            btn += "</buttons>";
            arr[i] = $(btn);
            arr[i].click(conf.buttons[i].click);
        }
        return arr;
    }
}
...
/* ActiveForm Event */
/*
 * ActiveForm 生成的表单防止二次提交，默认控制表单中的submit按扭，如果不想被控制，可以添加undisabled=undisabled属性来阻止
 */
$(document).ready(function(){
   $(document).bind('beforeSubmit',function(eve){
        if('undisabled' === $(eve.target).find("button:submit").attr('undisabled')){
            return true;
        }
        if('disabled' == $(eve.target).find("button:submit").attr('disabled')){
            return false;
        }
        $(eve.target).find("button:submit").attr('disabled','disabled').html('提交中...');
   }); 
});
/* End ActiveForm Event */
